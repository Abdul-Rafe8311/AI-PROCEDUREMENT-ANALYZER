...
test('TA FORM: the A3 reference layout — both supplier blocks on page 1, sign-off on page 2', async () => {
  // On A4 this was two pages with one supplier block each. A3 fits BOTH blocks on
  // the first page, which is the point of the paper change — the grid is read in
  // one view and only the sign-off spills over. Farid confirmed a second page for
  // the approval blocks is fine; what must not happen is the GRID splitting.
  const doc = await PDFDocument.load(await renderFlat());
  assert.equal(doc.getPageCount(), 2, 'grid on page 1, sign-off on page 2');
  ...
  assert.match(page1, /Suppliers 5[–-]5 of 5/, 'page 1 also carries supplier 5');
  ...
  // Every term row travels with its own supplier group — both groups are on page 1,
  // so each label appears there twice rather than once per page.
  for (const label of ['Total Price without VAT', 'Payment Terms', 'Delivery Time', 'Delivery Terms', 'Country of Origin', 'Warranty', 'Technical Comments']) {
    assert.equal(page1.split(label).length - 1, 2, `page 1 has "${label}" once per supplier block`);
  }
...
test('TA FORM: the output is a real AcroForm — editable, uniquely named, pre-filled', async () => {
  ...
  // Names are page-scoped and stable. On A3 both supplier blocks live on page 1,
  // so the value fields are all p1.* and page 2 carries only the sign-off.
