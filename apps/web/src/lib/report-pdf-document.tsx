...
function TechnicalApprovalPage({
  pr,
  match,
  dateStr,
}: {
    ...
    <Page size="A4" style={s.page}>
      <View>
        <Text style={s.sectionTitle}>
          Technical Approval — Purchase Requisition Matching
        </Text>
        <Text style={s.para}>
          Each supplier&apos;s quoted items are matched against the company Purchase Requisition
          {pr.requestNo ? ` ${pr.requestNo}` : ''} ({pr.items.length} item{pr.items.length === 1 ? '' : 's'}) by
          description &amp; spec. &quot;Approved&quot; means the item was matched to a requisitioned item; the final
          technical accept/reject remains a human decision.
        </Text>

        {/* Per-supplier match signal */}
          ...
          {match.bySupplier.map((sm) => {
            ...
            return (
              <View key={sm.quotationId} style={s.recRow}>
                ...
                <Text style={[s.recValue, { color: sm.allMatched ? C.success : C.warning }]}>
                  Items match PR: {sm.allMatched ? 'Yes' : `No — ${bits.join(', ')}`}
                </Text>
              </View>
            );
          })}
        ...
        {/* PR-item x supplier grid */}
        <View style={{ marginTop: 12 }}>
          <View style={s.thead}>
            <Text style={[s.th, { flex: 2.6 }]}>Requisition item</Text>
            <Text style={[s.th, { flex: 0.9, textAlign: 'right' }]}>Req. qty</Text>
            {match.bySupplier.map((sm) => (
              <Text key={sm.quotationId} style={[s.th, { flex: supCol, textAlign: 'center' }]}>
                {sm.supplier}
              </Text>
            ))}
          </View>
          {pr.items.map((it, ri) => (
            <View key={ri} style={ri % 2 ? [s.row, s.rowAlt] : s.row} wrap={false}>
              ...
              <Text style={[s.td, { flex: 0.9, textAlign: 'right', color: C.muted }]}>
...
export function ReportDocument({ analysis, fx }: { analysis: AnalysisResult; fx: FxRates | null }) {
  ...
  return (
    <Document title="Procurement Analysis Report" author="AI Procurement Copilot">
      ...
      {/* ── Technical Approval (only when a company PR was matched) ── */}
      {analysis.purchaseRequisition && analysis.prMatch && (
        <TechnicalApprovalPage
