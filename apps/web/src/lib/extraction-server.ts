...
const EXTRACTION_SYSTEM_PROMPT = [
  ...
  '',
  'ONE PRICED ROW = ONE LINE ITEM. Return each priced product as its OWN line item;',
  'NEVER combine two DIFFERENT products into one line — not when they sit on adjacent',
  'rows, not when a description wraps onto a second visual line, and not when the text',
  'layer scattered a table into columns. If a candidate "name" contains TWO distinct',
  'product descriptions — e.g. two different anchor types, or two different part codes',
  '/ drawing numbers each with its own size or grade — it is TWO line items; split it,',
  'keeping each with its own quantity and unit price. A drawing or standard reference',
  '("ACC DRWG", "AS PER DRG", "DRG No.", "as per drawing") belongs to the single item',
  'it annotates and must NOT cause two items to be merged. Each row that carries its',
  'own quantity and/or unit price is a separate line item.',
].join('\n');
...
const PR_EXTRACTION_SYSTEM_PROMPT = [
  ...
  '                                //   copied VERBATIM and IN FULL (e.g. the entire',
  '                                //   "Anchors for Kiln department" phrase — NOT just the',
  '                                //   first word "Anchors"), taken from a header',
  '                                //   "Description"/"Subject"/"Purpose"/"PR Description"/',
  '                                //   "Required for" field, OR an unlabeled purpose/title',
  '                                //   line near the top (e.g. a "... for ... department"',
  '                                //   phrase). NOT a line item. null only if truly absent.',
